...
class EigenIsApprox : public Catch::MatcherBase<T> {
 ...
 public:
  ...
  {
    std::ostringstream ss;
    ss << std::endl << "    is not approximately equal to" << std::endl << value.format(HeavyFmt);
...
template <typename T>
inline EigenIsApprox<T> IsApprox(T value, boost::optional<typename T::Scalar> precision = boost::none)
{
