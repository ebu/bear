...
def flow(basic_config):
    return make_flow(basic_config, visr_bear.DirectSpeakersGainCalc)


@pytest.fixture(scope="module")
def blocks_and_gain_vectors(basic_config):
    flow = make_flow(basic_config, visr_bear.DirectSpeakersGainCalc)
    ...
    for azimuth in [0, 30, 60]:
        ...
        metadata_in.enqueue(visr_bear.ADMParameterDirectSpeakers(0, oi))
        flow.process()
...
def push(flow):
    ...
    def f(block, **kwargs):
        block = visr_bear.api.DirectSpeakersInput(block)
        for key, value in kwargs.items():
            setattr(block, key, value)
        metadata_in.enqueue(visr_bear.ADMParameterDirectSpeakers(0, block))
