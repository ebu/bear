...
namespace ear_bits {
  namespace hoa {
    ...
    {
      return std::sqrt((2.0 * n + 1.0) * boost::math::factorial<double>(n - abs_m) /
                       boost::math::factorial<double>(n + abs_m));
    }

    inline double norm_SN3D(int n, int abs_m)
    {
      return std::sqrt(boost::math::factorial<double>(n - abs_m) / boost::math::factorial<double>(n + abs_m));
    }
    ...
    double sph_harm(int n, int m, double az, double el, NormT norm)
    {
      ...
      return norm(n, std::abs(m)) * Alegendre(n, std::abs(m), std::sin(el)) * scale;
    }
                                ...
                                NormT norm)
    {
      ...
      for (Eigen::Index point_i = 0; point_i < points.rows(); point_i++) {
        for (Eigen::Index coeff_i = 0; coeff_i < n.size(); coeff_i++) {
          ...
          double el = std::atan2(points(point_i, 2), std::hypot(points(point_i, 0), points(point_i, 1)));
          Y_virt(coeff_i, point_i) = sph_harm(n(coeff_i), m(coeff_i), az, el, norm);
        }
      }
    ...
    {
      Eigen::Index n_speakers = panning_function(Eigen::Vector3d{0, 1, 0}).get().size();
    ...
    /// input vectors is 1
    inline void normalize_decode_matrix(Eigen::MatrixXd &D, Eigen::MatrixXd Y_virt)
    {
                                                    ...
                                                    NormFromT norm_from)
    {
      ...
      for (Eigen::Index coeff_i = 0; coeff_i < n.size(); coeff_i++)
        conversion(coeff_i) =
            norm_to(n(coeff_i), std::abs(m(coeff_i))) / norm_from(n(coeff_i), std::abs(m(coeff_i)));
    ...
    Eigen::MatrixXd allrad_design(Eigen::Matrix<double, Eigen::Dynamic, 3> points,
                                  PanningFuncT panning_function,
                                  Eigen::VectorXi n,
                                  Eigen::VectorXi m,
                                  NormT norm)
